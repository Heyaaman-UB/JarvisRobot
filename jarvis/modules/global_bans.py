...
import jarvis.modules.sql.global_bans_sql as sql
from jarvis import (
    dispatcher,
    OWNER_ID,
    SUDO_USERS,
...
from jarvis.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from jarvis.modules.helper_funcs.misc import send_to_list
from jarvis.modules.sql.users_sql import get_all_chats
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if GBAN_LOGS:
        ...
        send_to_list(
            bot,
            SUDO_USERS + SUPPORT_USERS,
            f"Gban complete! (User banned in {gbanned_chats} chats)",
        )
    ...
    if gban_time > 60:
        ...
        message.reply_text(
            f"Done! This gban affected {gbanned_chats} chats, Took {gban_time} min"
        )
    else:
        message.reply_text(
            f"Done! This gban affected {gbanned_chats} chats, Took {gban_time} sec"
        )
