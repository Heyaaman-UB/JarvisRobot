...
import jarvis.modules.sql.welcome_sql as sql
from jarvis import (
    dispatcher,
    OWNER_ID,
    DEV_USERS,
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                ...
                welcome_log = (
                    ...
                    f"#USER_JOINED\n"
                    f"Haha my Owner just joined the chat"
                )

            # Welcome Devs
            elif new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Whoa! A member of the Jarvis Support just joined!"
                )

            # Welcome Sudos
            elif new_mem.id in SUDO_USERS:
                update.effective_message.reply_text(
                    "Huh! A Friendly User just joined! Stay Alert!"
                )

            # Welcome Support
            elif new_mem.id in SUPPORT_USERS:
                update.effective_message.reply_text(
                    "Huh! Someone with a Support User has just joined!"
                )

            # Welcome Whitelisted
            elif new_mem.id in TECHY_USERS:
                update.effective_message.reply_text(
                    "Oof! A Techy User just joined!"
                )

            # Welcome Sardegnas
            elif new_mem.id in WHITELIST_USERS:
                update.effective_message.reply_text(
                    "Oof! A Speacial Member just joined!"
                )

            # Welcome yourself
            elif new_mem.id == bot.id:
                update.effective_message.reply_text("Thanks for adding me! Join our support chat @CFC_BOT_support for support.. <3")
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            # Give the devs a special goodbye
                ...
                update.effective_message.reply_text("See you later at the Jarvis Support!")
                return
